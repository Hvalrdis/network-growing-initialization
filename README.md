...
|---|---|---|
| Mode A — Column-Zero Initialization | zero `new1`; sample `new2` and `new3` with the grown fan-in | Yes |
| Mode B — Row-First Column-Zero Initialization | sample `new2` with the inherited fan-in; zero `new1` and `new3` | Yes |
| Mode C — Row-Zero Initialization | sample `new1`; zero `new2` and `new3` | Yes |
| Mode D — Homogeneous Initialization | initialize all added blocks with the default grown-layer rule | No |
| Mode E — Homogeneous Initialization with Empirical Variance | sample all added blocks using the inherited weight variance | No |

For ReLU networks, Mode C uses the convention $\phi'(0)=1$ so that zero-valued
added coordinates can still receive gradients. Existing biases are retained,
and newly added biases are zero in the function-preserving modes. Homogeneous
modes follow the corresponding backbone-specific initialization rule; the
classification bias remains unchanged.

When a widened convolution is followed by BatchNorm, inherited statistics and
affine parameters are retained. Added channels use scale 1, shift 0, running
mean 0, and running variance 1.
...
RandAugment, ImageNet normalization, random erasing, and batch-level Mixup or
CutMix. Its Mixup/CutMix probability is 1.0, with alphas 0.8/1.0 and a 0.5
switch probability. All CvT modes use the 100-epoch schedule and preserve
inherited AdamW state across growth. GradMax is run for MLP, VGG-11, and
WRN-28-1; it is not part of the ViT or CvT-13 comparison.
...
3.10.12, PyTorch 2.1.2, torchvision 0.16.2, timm 0.4.12, NumPy 1.26.3, and
Matplotlib 3.8.0.
...
- train/test accuracy and loss curves against both wall-clock time and epoch;
- parameter count against epoch;
- tabular result files using the manuscript labels.

The generated figures are `train_accuracy_vs_time.png`,
`test_accuracy_vs_time.png`, `train_loss_vs_time.png`,
`test_loss_vs_time.png`, and their four corresponding `*_vs_epoch.png` files,
together with `parameter_count_vs_epoch.png`.
...
layout of Modes A–E, final-width plans, the CvT-13 augmentation and mixing
configuration, CvT-13 parameter count, Grow-d/Grow-H layouts, and
optimizer-state migration. It also generates the complete plot set in a
temporary directory and verifies every output file. Passing this suite checks
structural correctness; it does not replace the complete 80/100/200-epoch runs.
