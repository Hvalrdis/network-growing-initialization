...
Grow-H (B/D). See [the ablation notes](ablations/vit_growth_axis/README.md).
...
[the ablation notes](ablations/optimizer_state/README.md).
...
- comparison plots and tabular result files.
...
├── model_mlp.py                    # architecture-specific adapters
├── model_vgg.py
├── model_wrn.py
├── model_vit.py
├── model_cvt.py
...
├── PARITY.md                       # migration and source-parity notes
├── verify.py                       # no-dataset structural regression suite
└── requirements.txt
```

The model-specific files contain complete construction and growth logic; the
...
read [PAPER_MAPPING.md](PAPER_MAPPING.md). For migration
checks, read [PARITY.md](PARITY.md).
...
This check requires no dataset. It verifies model construction, all growth
modes, final-width plans, NZZ zero/nonzero blocks, CvT-13 parameter count,
Grow-d/Grow-H layouts, and optimizer-state migration. Passing this suite checks
