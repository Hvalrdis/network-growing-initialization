...
def growth_plan(seed_model: CvT, big_model: CvT, grow_steps: int) -> list[list[int]]:
    ...
    for stage, heads in enumerate(STAGE_HEADS):
        old_head = seed_model.arch["dims"][stage] // heads
        final_head = big_model.arch["dims"][stage] // heads
        base, remainder = divmod(final_head - old_head, grow_steps)
...
def _copy_matrix_weight(old, new, heads_out: int, heads_in: int, mode: str) -> None:
    ...
    if mode == "mode_a":
        ...
        _kaiming_uniform_region(w_new1, int(new.shape[1]) * receptive)
    elif mode == "mode_d":
        pass
    elif mode == "mode_e":
        std = _safe_std(old)
...
def _copy_vector_weight(
    old, new, heads: int, mode: str, axis: int = 0, zero_new: bool = True
) -> None:
    if zero_new or mode == "mode_c":
        _copy_vector_state(old, new, heads, axis)
    else:
        old_size, new_size = int(old.shape[axis]), int(new.shape[axis])
        old_hd, new_hd = old_size // heads, new_size // heads
        old_shape, new_shape = list(old.shape), list(new.shape)
        ...
        index[axis + 1] = slice(0, old_hd)
        new_view[tuple(index)].copy_(old_view)
    if mode == "mode_e" and not zero_new:
        old_size, new_size = int(old.shape[axis]), int(new.shape[axis])
        old_hd, new_hd = old_size // heads, new_size // heads
...
def _transfer_cvt(old_model: CvT, new_model: CvT, mode: str) -> None:
    ...
    def vector(old_p, new_p, heads: int, axis: int = 0, zero_new: bool = True):
        _copy_vector_weight(old_p.data, new_p.data, heads, mode, axis, zero_new)
        registry.append((old_p, new_p, _vector_copier(heads, axis)))
        handled.add(new_p)
    ...
    def depthwise(
        ...
        if old_conv.bias is not None:
            vector(old_conv.bias, new_conv.bias, heads_in)
        batchnorm(old_dw.net[1], new_dw.net[1])
        matrix(old_dw.net[2].weight, new_dw.net[2].weight, heads_out, heads_in)
        if old_dw.net[2].bias is not None:
            vector(old_dw.net[2].bias, new_dw.net[2].bias, heads_out)
    ...
    for stage_index, (old_stage, new_stage) in enumerate(
        ...
        vector(old_stage[0].bias, new_stage[0].bias, heads_out)
        vector(
            old_stage[1].g, new_stage[1].g, heads_out, axis=1, zero_new=mode == "mode_c"
        )
        vector(old_stage[1].b, new_stage[1].b, heads_out, axis=1)
        ...
        for old_block, new_block in zip(old_stage[2].layers, new_stage[2].layers):
            ...
            for old_norm, new_norm in (
                ...
                vector(
                    old_norm.g, new_norm.g, heads_out, axis=1, zero_new=mode == "mode_c"
                )
                vector(old_norm.b, new_norm.b, heads_out, axis=1)
            ...
            vector(
                old_attention.to_out[0].bias, new_attention.to_out[0].bias, heads_out
            )

            old_ff, new_ff = old_ff_norm.fn, new_ff_norm.fn
            matrix(old_ff.net[0].weight, new_ff.net[0].weight, heads_out, heads_out)
            vector(old_ff.net[0].bias, new_ff.net[0].bias, heads_out)
            matrix(old_ff.net[3].weight, new_ff.net[3].weight, heads_out, heads_out)
            vector(old_ff.net[3].bias, new_ff.net[3].bias, heads_out)

    matrix(old_model.head.weight, new_model.head.weight, 1, STAGE_HEADS[-1])
    new_model.head.bias.copy_(old_model.head.bias)
    registry.append(
        (old_model.head.bias, new_model.head.bias, lambda old, new: new.copy_(old))
    )
    handled.add(new_model.head.bias)
    ...
    for name, new_parameter in new_model.named_parameters():
        ...
        if old_parameter.shape == new_parameter.shape:
            ...
            registry.append(
                (old_parameter, new_parameter, lambda old, new: new.copy_(old))
            )
...
def compute_training_loss(model, x: torch.Tensor, y: torch.Tensor, loss_fn: nn.Module):
    ...
    # Mixup and CutMix parameters follow the CvT training protocol.
    ...
    logits = model(mixed)
    return logits, lam * loss_fn(logits, y) + (1.0 - lam) * loss_fn(
        logits, y[permutation]
    )
