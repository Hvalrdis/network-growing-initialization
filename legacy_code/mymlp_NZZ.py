...
class MLP(nn.Module):
    ...
    def expand_layer_nzz(self, layer_index, num_new_neurons, last_nb_increase=0):
        """Expand one hidden layer with the NZZ rule.

        Matrix block convention for the layer being updated / its next layer:
            W_new1: old outputs receiving newly added inputs -> random (N)
            W_new2: new outputs receiving old inputs         -> zero   (Z)
            W_new3: new outputs receiving new inputs         -> zero   (Z)

        For an MLP, when layer `layer_index` is widened:
        - the newly added rows of the current hidden layer are set to zero;
        - the newly added input columns of the following layer are randomly initialized.
        Therefore the new hidden units produce zero activation at the growth instant,
        while the next layer already has trainable random connections from them.
        """
        if int(num_new_neurons) <= 0:
            return

        if layer_index < 0 or layer_index >= len(self.layers):
            raise IndexError(f"layer_index={layer_index} is out of range for {len(self.layers)} hidden layers")
        ...
        new_weight[:old_out_features, :] = old_weight
        # NZZ: W_new2 = 0 and W_new3 = 0.  The whole newly added output-row
        # block is already zero, including the columns that came from the
        # previous layer's most recent growth.

        expanded_layer = nn.Linear(in_features, new_out_features, bias=self.with_bias).to(old_weight.device)
        expanded_layer.weight = nn.Parameter(new_weight)

        if self.with_bias:
            old_bias = old_layer.bias.data
            new_bias = torch.zeros(new_out_features, device=old_bias.device, dtype=old_bias.dtype)
            new_bias[:old_out_features] = old_bias
            expanded_layer.bias = nn.Parameter(new_bias)

        self.layers[layer_index] = expanded_layer

        # Expand the following layer's input dimension.  This is W_new1 and is
        # random for NZZ.  If the grown hidden layer is the last hidden layer,
        # the following layer is the output layer.
        next_layer = self.layers[layer_index + 1] if layer_index < len(self.layers) - 1 else self.output_layer
        if not isinstance(next_layer, nn.Linear):
        ...
        print(
            f"mode_NZZ layer {layer_index}: W_new2=0, W_new3=0; "
            f"next W_new1=random, std={std}"
        )
    ...
    def growGradMax(self, nbToGrow: list = None):
        ...
        for l in range(0, len(fc_layers)):
            ...
            if getattr(fc, "inputsToAdd", None) is not None:
                ...
                if grow is None:
                    raise RuntimeError("grow module is required only for GradMax, but it is not available")
