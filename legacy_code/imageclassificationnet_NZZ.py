...
class SimpleChainImageClassificationNetwork(torch.nn.Module):
    ...
    def _mode_to_pattern(self, mode):
        if isinstance(mode, str):
            m = mode.strip().upper()
            if m.startswith("YIFAN"):
                m = m[5:]
            if m in {"NNN", "NZZ", "ZNN", "ZNZ"}:
                return m
            if m in {"N", "Z"}:
                return m
            raise ValueError(f"Unknown mode pattern: {mode}")
    ...
    def _fill_tensor_by_code(self, tensor, code, std):
        ...
        if code == "Z":
            ...
            raise ValueError(f"Unknown initialization code: {code}")
    ...
    def _expand_associated_batchnorm(self, layer_index, old_out_channels, new_out_channels):
        ...
        with torch.no_grad():
            ...
            if old_batchnorm.track_running_stats:
                ...
                new_batchnorm.num_batches_tracked.copy_(old_batchnorm.num_batches_tracked)

        self.backbone[bn_index] = new_batchnorm.to(self.device)

    def _expand_current_conv_outputs(self, layer_index, nb_increase, last_nb_increase, pattern):
        layer = self.backbone[layer_index]
        if not isinstance(layer, torch.nn.Conv2d):
            raise ValueError(f"Layer {layer_index} is not a convolutional layer.")

        pattern = self._mode_to_pattern(pattern)
        mode_name = f"mode_{pattern}"
        _, code_wnew2, code_wnew3 = pattern
        ...
        with torch.no_grad():
            ...
            prev_added_in_channels = int(max(0, min(int(last_nb_increase), total_in_channels)))
            ...
            if old_out_channels < new_out_channels:
                std_wnew2 = self._conv_normal_std(
                    old_input_channels if pattern == "ZNZ" else total_in_channels,
                    layer.kernel_size,
                )
                std_wnew3 = self._conv_normal_std(total_in_channels, layer.kernel_size)

                self._fill_tensor_by_code(
                ...
                print(
                    f'{mode_name} layer {layer_index}: '
                    f'W_new2={code_wnew2}, std_W_new2={std_wnew2}; '
                    f'W_new3={code_wnew3}, std_W_new3={std_wnew3}'
                )

            if old_bias is not None:
                new_bias = torch.zeros(new_out_channels, device=self.device, dtype=old_bias.dtype)
                new_bias[:old_out_channels] = old_bias
                new_layer.bias.data = new_bias

            self.backbone[layer_index] = new_layer.to(self.device)
            self._expand_associated_batchnorm(layer_index, old_out_channels, new_out_channels)

    def _expand_next_conv_inputs(self, layer_index, nb_increase, pattern):
        ...
        with torch.no_grad():
            ...
            new_next_layer.weight.data[:, :old_in_channels, :, :] = old_weight_next_layer
            ...
            print(
                f'{mode_name} layer {next_conv_index}: '
                f'W_new1={code_wnew1}, std={std}'
            )
    ...
    def _expand_conv_layer_with_pattern(self, layer_index, nb_increase, pattern, last_nb_increase=None):
    ...
    def expand_conv_layer(self, layer_index, nb_increase):
        self._expand_conv_layer_with_pattern(
            layer_index=layer_index,
            nb_increase=nb_increase,
            pattern="NNN",
            last_nb_increase=None,
        )
    ...
    def adjust_classifier(self, new_in_features, mode):
        ...
        init_code = self._mode_to_pattern(mode)
        if len(init_code) == 3:
        ...
        init_weight = torch.empty(init_shape, device=self.device, dtype=old_weight.dtype)
        std = self._linear_normal_std(new_in_features)
        self._fill_tensor_by_code(init_weight, init_code, std)
        print(f'classifier first layer: W_new1={init_code}, std={std}')
    ...
    def expand_conv_layer_mode2(self, layer_index, nb_increase, last_nb_increase):
        self._expand_conv_layer_with_pattern(
            ...
            nb_increase=nb_increase,
            pattern="NZZ",
            last_nb_increase=last_nb_increase,
        )
