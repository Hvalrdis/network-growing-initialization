...
import imageclassificationnet as imageclassificationnet
...
def test_expand_layer_mode1(net, config_add):
    ...
    increases = config_add

    for i, layer_index in enumerate(conv_indices):
        if i < len(increases):
            nb_increase = increases[i]
        else:
            nb_increase = 0
...
def test_expand_layer_mode2(net_mode2, config_add):
    ...
    conv_indices = [i for i, layer in enumerate(net_mode2.backbone) if isinstance(layer, torch.nn.Conv2d)]
    increases = config_add
    last_increase = 0
    print(f'mode2 conv_indices = {conv_indices}')
    ...
    for i, layer_index in enumerate(conv_indices):
        ...
        if nb_increase > 0:
            net_mode2.expand_conv_layer_mode2(layer_index, nb_increase, last_increase)
...
def test_expand_layer_mode3(net_mode3, config_add):
    ...
    conv_indices = [i for i, layer in enumerate(net_mode3.backbone) if isinstance(layer, torch.nn.Conv2d)]
    increases = config_add
    
    print(f'mode3 conv_indices = {conv_indices}')
    ...
    for i, layer_index in enumerate(conv_indices):
        ...
        if nb_increase > 0:
            net_mode3.expand_conv_layer_mode3(layer_index, nb_increase)
                
    return net_mode3

def test_expand_layer_mode4(net_mode4, config_add):

    conv_indices = [i for i, layer in enumerate(net_mode4.backbone) if isinstance(layer, torch.nn.Conv2d)]
    increases = config_add
    ...
    for i, layer_index in enumerate(conv_indices):
        ...
        if nb_increase > 0:
            net_mode4.expand_conv_layer_mode4(layer_index, nb_increase) 

    return net_mode4


def test_expand_layer_mode5(net_mode5, config_add, loaderTrain, loss_fn, grow_batch_size=128):
    gradmax_grow(
        net=net_mode5,
        add_per_conv=list(config_add),
...
def gradmax_grow(
    ...
    conv_cnt = len(net.convIdx)
    if conv_cnt == 0:
        return

    nbToGrow = [0 for _ in range(conv_cnt)]
    for i in range(min(len(add_per_conv), conv_cnt)):
        nbToGrow[i] = int(add_per_conv[i])

    if len(add_per_conv) < conv_cnt:
        nbToGrow[-1] = 0
...
def build_vgg11_seed_net(
    ...
    # main_mode5: blocklist = [[1],[2],[4,4],[8,8],[8,8]], base_width=64*wm
...
def run_compare(seed: int, run_id: int = 1):
    ...
    output_dir = f"outputs_compare/cifar100/{time.strftime('%Y%m%d_%H%M%S')}_run{run_id:02d}_seed{seed}_lr{lr}"
    ...
    model_c = build_vgg11_seed_net(nbClasses, device=device, with_batchnorm=False, width_multiplier=1.0).to(device)
    ...
    optimizer_c, scheduler_c = make_opt_sch(model_c)
    optimizer_seed, scheduler_seed = make_opt_sch(model_seed)
    
    optimizer_a = optimizer_b = optimizer_d = optimizer_e = optimizer_f = None
    scheduler_a = scheduler_b = scheduler_d = scheduler_e = scheduler_f = None
    ...
    def _train_one_step(
        tag, net, optimizer, scheduler, x, y,
        global_step, grow_done, grow_impl
    ):
        net.train()
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for x, y in loaderTrain:
            ...
            if model_a is None:
                ...
                if global_step_seed == grow_start_iter:
                    ...
                    optimizer_f, scheduler_f, grow_done_f = _maybe_grow_only(
                        "f/mode5(GradMax)", model_f, optimizer_f, scheduler_f, global_step_f, grow_done_f, grow_impl_mode5
                    )
            ...
            loss_sum, corr, bs, optimizer_f, scheduler_f, global_step_f, grow_done_f = _train_one_step(
                "f/mode5(GradMax)", model_f, optimizer_f, scheduler_f, x, y, global_step_f, grow_done_f, grow_impl_mode5
            )
    ...
    with open(filename, "w") as f:
        ...
        for i in range(num_epochs):
            f.write(f"{i+1}\tc\t{train_losses_c[i]:.6f}\t{train_accs_c[i]:.6f}\t{test_losses_c[i]:.6f}\t{test_accuracies_c[i]:.6f}\t{time_record_c[i]:.2f}\t{avg_param_counts_c[i]}\n")

            f.write(f"{i+1}\ta\t{train_losses_a[i]:.6f}\t{train_accs_a[i]:.6f}\t{test_losses_a[i]:.6f}\t{test_accuracies_a[i]:.6f}\t{time_record_a[i]:.2f}\t{avg_param_counts_a[i]}\n")
            f.write(f"{i+1}\tb\t{train_losses_b[i]:.6f}\t{train_accs_b[i]:.6f}\t{test_losses_b[i]:.6f}\t{test_accuracies_b[i]:.6f}\t{time_record_b[i]:.2f}\t{avg_param_counts_b[i]}\n")
            f.write(f"{i+1}\td\t{train_losses_d[i]:.6f}\t{train_accs_d[i]:.6f}\t{test_losses_d[i]:.6f}\t{test_accuracies_d[i]:.6f}\t{time_record_d[i]:.2f}\t{avg_param_counts_d[i]}\n")
            f.write(f"{i+1}\te\t{train_losses_e[i]:.6f}\t{train_accs_e[i]:.6f}\t{test_losses_e[i]:.6f}\t{test_accuracies_e[i]:.6f}\t{time_record_e[i]:.2f}\t{avg_param_counts_e[i]}\n")
            f.write(f"{i+1}\tf\t{train_losses_f[i]:.6f}\t{train_accs_f[i]:.6f}\t{test_losses_f[i]:.6f}\t{test_accuracies_f[i]:.6f}\t{time_record_f[i]:.2f}\t{avg_param_counts_f[i]}\n")
