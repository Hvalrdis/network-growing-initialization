...
class SimpleChainImageClassificationNetwork(torch.nn.Module):
    ...
    def _expand_current_conv_outputs(
        self, layer_index, nb_increase, last_nb_increase, pattern
    ):
    ...
    def _expand_conv_layer_with_pattern(
        self, layer_index, nb_increase, pattern, last_nb_increase=None
    ):
...
def grow_model(
    ...
    for position, layer_index in enumerate(conv_indices):
        ...
        if mode == "mode_a":
            model._expand_conv_layer_with_pattern(
                layer_index, amount, "ZNN", previous_addition
            )
        elif mode == "mode_b":
            model._expand_conv_layer_with_pattern(
                layer_index, amount, "ZNZ", previous_addition
            )
        elif mode == "mode_c":
            model._expand_conv_layer_with_pattern(
                layer_index, amount, "NZZ", previous_addition
            )
        elif mode == "mode_d":
            model._expand_conv_layer_with_pattern(
                layer_index, amount, "NNN", previous_addition
            )
        elif mode == "mode_e":
