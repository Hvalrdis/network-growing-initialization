...
MODEL_MODULES = {
    "mlp": "unified_experiments.models.mlp",
    "vgg": "unified_experiments.models.vgg",
    "wrn": "unified_experiments.models.wrn",
    "vit": "unified_experiments.models.vit",
    "cvt": "unified_experiments.models.cvt",
}
...
def run_one(
    ...
    seed_model = (
        adapter.build_model(num_classes, device, config.seed_width, seed)
        if mode_order
        else None
    )
    plan = (
        adapter.growth_plan(seed_model, target_model, config.grow_steps)
        if seed_model is not None
        else []
    )
    if mode_order and len(plan) != config.grow_steps:
