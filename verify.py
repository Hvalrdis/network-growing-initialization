...
from . import model_cvt, model_mlp, model_vgg, model_vit, model_wrn
from .ablations.optimizer_state import experiment as optimizer_ablation
from .ablations.optimizer_state import model as optimizer_model
from .ablations.vit_growth_axis import model as vit_axis
from .experiment import generate_seeds, make_config
...
def _check_nzz() -> None:
    vit = model_vit.build_model(10, torch.device("cpu"), 0.125, 2)
    grown_vit = model_vit.grow_model(vit, "mode_c", [4], None, None, None, 0)
    old_weight = vit.encoder.layers[0].self_attention.out_proj.weight
    new_weight = grown_vit.encoder.layers[0].self_attention.out_proj.weight
    blocks = _new_blocks(old_weight, new_weight, 4, 4)
    assert torch.count_nonzero(blocks[0]) > 0
    assert torch.count_nonzero(blocks[1]) == 0 and torch.count_nonzero(blocks[2]) == 0

    cvt = model_cvt.build_model(10, torch.device("cpu"), 0.125, 2)
    grown_cvt = model_cvt.grow_model(cvt, "mode_c", [1, 3, 6], None, None, None, 0)
    old_weight = cvt.layers[2][2].layers[0][0].fn.to_out[0].weight
    new_weight = grown_cvt.layers[2][2].layers[0][0].fn.to_out[0].weight
    blocks = _new_blocks(old_weight, new_weight, 6, 6)
    assert torch.count_nonzero(blocks[0]) > 0
    assert torch.count_nonzero(blocks[1]) == 0 and torch.count_nonzero(blocks[2]) == 0
...
def main() -> None:
    ...
    assert list(model_mlp.MODE_SPECS) == EXPECTED_MODES_WITH_GRADMAX
    assert list(model_vgg.MODE_SPECS) == EXPECTED_MODES_WITH_GRADMAX
    assert list(model_wrn.MODE_SPECS) == EXPECTED_MODES_WITH_GRADMAX
    assert list(model_vit.MODE_SPECS) == EXPECTED_TRANSFORMER_MODES
    assert list(model_cvt.MODE_SPECS) == EXPECTED_TRANSFORMER_MODES
    ...
    _check_presets()
    print("All unified model checks passed.")
