...
def growth_plan(
    ...
    if grow_steps <= 0:
        raise ValueError("ViT growth-axis grow_steps must be positive")
    difference = int(big_model.config["hidden_dim"]) - int(
        seed_model.config["hidden_dim"]
    )
    if difference <= 0:
        raise ValueError("ViT growth-axis target width must exceed the seed width")
...
def validate_growth_plan(
    ...
    selected_axes = {mode.split("_")[1] for mode in selected_modes}
    if not selected_axes:
        return
    grow_steps = len(plan)
    old_hidden = int(seed_model.config["hidden_dim"])
    final_hidden = int(big_model.config["hidden_dim"])
    old_heads = int(seed_model.config["num_heads"])
    old_head_dim = old_hidden // old_heads

    valid_counts: set[int] | None = None
    if "d" in selected_axes:
        per_head_increase = (final_hidden - old_hidden) // old_heads
        valid_counts = {
            count
            for count in range(1, per_head_increase + 1)
            if per_head_increase % count == 0
        }
    if "h" in selected_axes:
        added_heads = (final_hidden - old_hidden) // old_head_dim
        head_counts = {
            count for count in range(1, added_heads + 1) if added_heads % count == 0
        }
        valid_counts = (
            head_counts if valid_counts is None else valid_counts & head_counts
        )

    if valid_counts is None or grow_steps not in valid_counts:
        axis_labels = {"d": "Grow-d", "h": "Grow-H"}
        axes = "/".join(axis_labels[axis] for axis in sorted(selected_axes))
        valid = ", ".join(map(str, sorted(valid_counts or set())))
        raise ValueError(
            f"grow_steps={grow_steps} is incompatible with {axes}. Grow-d must "
            "add whole per-head dimensions and Grow-H must add whole attention "
            f"heads at every event. Valid grow_steps: {valid}."
        )
