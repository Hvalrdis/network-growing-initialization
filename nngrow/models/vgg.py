...
def _validate_grow_steps(differences: Sequence[int], grow_steps: int) -> None:
    if grow_steps <= 0:
        raise ValueError("VGG grow_steps must be positive")
    if any(difference < 0 for difference in differences):
        raise ValueError("VGG target widths must not be smaller than seed widths")
    positive = [difference for difference in differences if difference > 0]
    if not positive:
        raise ValueError(
            "VGG seed and target widths are identical; no growth is possible"
        )
    maximum = min(positive)
    if grow_steps > maximum:
        raise ValueError(
            f"VGG grow_steps={grow_steps} is too large for convolution-width "
            f"differences {list(differences)}. Use at most {maximum} so every "
            "growth event adds at least one channel to each growable convolution."
        )
...
def validate_growth_plan(
    ...
    seed_convs = _conv_layers(seed_model)
    big_convs = _conv_layers(big_model)
    for position, ((layer_index, _), (_, big_layer)) in enumerate(
        zip(seed_convs, big_convs)
    ):
        difference = int(big_layer.out_channels) - int(
            seed_convs[position][1].out_channels
        )
        if difference <= 0:
            continue
        if position + 1 >= len(seed_convs):
            raise ValueError(
                "VGG GradMax cannot grow the final convolution because it has "
                "no downstream convolutional SVD target."
            )
        next_layer = seed_convs[position + 1][1]
        kernel_area = int(next_layer.kernel_size[0] * next_layer.kernel_size[1])
        rank_limit = int(next_layer.out_channels) * kernel_area
        requested = max(int(step[position]) for step in plan)
        if requested > rank_limit:
            minimum = math.ceil(difference / rank_limit)
            raise ValueError(
                f"VGG grow_steps is too small for GradMax at convolution "
                f"{layer_index}: one event requests {requested} channels, but "
                f"the downstream SVD rank limit is {rank_limit}. Use at least "
                f"{minimum} growth events or exclude gradmax."
            )
