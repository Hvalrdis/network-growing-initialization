...
def growth_plan(
    ...
    _validate_grow_steps(differences, grow_steps)
...
def _validate_grow_steps(differences: Sequence[int], grow_steps: int) -> None:
    if grow_steps <= 0:
        raise ValueError("WRN grow_steps must be positive")
    if any(difference < 0 for difference in differences):
        raise ValueError("WRN target widths must not be smaller than seed widths")
    positive = [difference for difference in differences if difference > 0]
    if not positive:
        raise ValueError(
            "WRN seed and target widths are identical; no growth is possible"
        )
    maximum = min(positive)
    if grow_steps > maximum:
        raise ValueError(
            f"WRN grow_steps={grow_steps} is too large for internal-width "
            f"differences {list(differences)}. Use at most {maximum} so every "
            "growth event adds at least one channel to each residual block."
        )


def validate_growth_plan(
    seed_model: WideResNetGradMax,
    big_model: WideResNetGradMax,
    plan: Sequence[Sequence[int]],
    selected_modes: Sequence[str],
) -> None:
    if "gradmax" not in selected_modes:
        return
    seed_tuples = seed_model.get_grow_layer_tuples()
    big_tuples = big_model.get_grow_layer_tuples()
    for position, (seed_tuple, big_tuple) in enumerate(zip(seed_tuples, big_tuples)):
        seed_conv1: nn.Conv2d = seed_tuple[0]
        seed_conv2: nn.Conv2d = seed_tuple[-1]
        difference = int(big_tuple[0].out_channels) - int(seed_conv1.out_channels)
        if difference <= 0:
            continue
        output_rank = int(seed_conv2.out_channels) * math.prod(seed_conv2.kernel_size)
        input_rank = int(seed_conv1.in_channels) * math.prod(seed_conv1.kernel_size)
        rank_limit = min(output_rank, input_rank)
        requested = max(int(step[position]) for step in plan)
        if requested > rank_limit:
            minimum = math.ceil(difference / rank_limit)
            raise ValueError(
                f"WRN grow_steps is too small for GradMax at residual block "
                f"{position}: one event requests {requested} channels, but the "
                f"SVD rank limit is {rank_limit}. Use at least {minimum} growth "
                "events or exclude gradmax."
            )
